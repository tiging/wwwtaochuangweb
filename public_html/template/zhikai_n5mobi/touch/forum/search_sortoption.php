...
<!--{loop $quicksearchlist $optionid $option}-->
        ...
        <!--{if getstatus($option['search'], 1)}-->
        	<!--{block formsearch}-->
	            <div class="sxtcc">
	                <div class="sxtccm">$option[title]:</div>
	                <!--{if in_array($option['type'], array('radio', 'checkbox', 'select', 'range'))}-->
	                    <div class="sxtccz" id="select_$option[identifier]">
	...
	<!--{if getstatus($option['search'], 2)}-->
    	<!--{block fontsearch}-->
        <tr class="sxtca"><th width="8%">$option[title]:</th></tr>
			<tr class="sxtcb">
...
<div>
	<!--{if $fontsearch_html}-->
	    <div class="ptn pbn bbs n5-fljs">
		    <table id="fontsearch" class="tsm cl n5-flsx">
		         $fontsearch_html
		    </table>
	    </div>
	<!--{/if}-->
	<!--{if $formsearch_html}-->
	    <form method="post" autocomplete="off" name="searhsort" id="searhsort" class="n5_flss bbs pns mfm cl" action="forum.php?mod=forumdisplay&fid=$_G[fid]&filter=sortid&sortid=$_GET[sortid]">
