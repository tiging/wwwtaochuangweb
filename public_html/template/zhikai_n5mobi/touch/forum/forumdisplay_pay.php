...
<div id="ct" class="n5_hyysmm cl">
	<div class="mn">
		<div class="nfl">
			<div class="f_c">
				<span class="srmmtsy">{lang youneedpay} $paycredits {$_G['setting']['extcredits'][$_G['setting']['creditstransextra'][1]]['unit']}{$_G['setting']['extcredits'][$_G['setting']['creditstransextra'][1]]['title']} {lang onlyintoforum}</span>
				<div class="srmmsrk">
					<form method="post" autocomplete="off" action="forum.php?mod=forumdisplay&fid=$_G[fid]&action=paysubmit">
						<input type="hidden" name="formhash" value="{FORMHASH}" />
						<button class="pn pnc vm" type="submit" name="loginsubmit" value="true">{lang confirmyourpay}</button>
						&nbsp;<button class="pn vm" type="button" onclick="history.go(-1)">{lang cancel}</button>
					</form>
				</div>
			</div>
		</div>
