 ...
 *
 *      $Id: viewthread.php 35157 2014-12-23 01:53:17Z 草_根_吧 $
 */
...
class xn_appserver_api {
	...
	function output() {
		...
		foreach ($GLOBALS['comments'] as $pid => $comments) {
			$comments = appserver_core::getvalues($comments, array('/^\d+$/'), array('id', 'tid', 'pid', 'author', 'authorid', 'dateline', 'comment', 'avatar'));
		...
		$variable = array(
		    'group' => appserver_core::getvalues($_G['group'], array('allowdelpost', 'allowbanpost', 'allowwarnpost', 'grouptitle')),
		    'thread' => $_G['thread'],
		    'fid' => $_G['fid'],
		    'postlist' => array_values(appserver_core::getvalues($GLOBALS['postlist'], array('/^\d+$/'), array('pid', 'tid', 'author', 'first', 'dbdateline', 'dateline', 'username', 'adminid', 'memberstatus', 'authorid', 'username', 'groupid', 'memberstatus', 'status', 'message', 'number', 'memberstatus', 'groupid', 'attachment', 'attachments', 'attachlist', 'imagelist', 'anonymous', 'position', 'rewardfloor', 'replycredit'))),
		...
		$threadsortshow = appserver_core::getvalues($GLOBALS['threadsortshow'], array('/^(?!typetemplate).*$/'));
		...
		foreach ($variable['postlist'] as $k => $post) {
			...
			$variable['postlist'][$k]['message'] = xn_appserver_api::_findimg($variable['postlist'][$k]['message']);
			if ($GLOBALS['aimgs'][$post['pid']]) {
				$imagelist = array();
				foreach ($GLOBALS['aimgs'][$post['pid']] as $aid) {
					$extra = '';
					$url = xn_appserver_api::_parseimg('', $GLOBALS['postlist'][$post['pid']]['attachments'][$aid]['url'] . $GLOBALS['postlist'][$post['pid']]['attachments'][$aid]['attachment'], '');
			...
			$variable['postlist'][$k]['groupiconid'] = appserver_core::usergroupIconId($post['groupid']);
		}
		...
		if (!empty($GLOBALS['debate'])) {
			$variable['firststand'] = $GLOBALS['firststand'];
		...
		if (!empty($GLOBALS['activity'])) {
			...
			$variable['special_activity']['joinfield'] = appserver_core::getvalues($setting, array('/./'), array('fieldid', 'formtype', 'available', 'title', 'formtype', 'choices'));
			...
			$variable['special_activity']['basefield'] = appserver_core::getvalues($GLOBALS['applyinfo'], array('message', 'payment'));
	...
	function _findimg($string) {
		return preg_replace('/(<img src=\")(.+?)(\".*?\>)/ise', "xn_appserver_api::_parseimg('\\1', '\\2', '\\3')", $string);
	}
